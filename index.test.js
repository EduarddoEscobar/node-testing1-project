...
describe('[Exercise 1] trimProperties', () => {
  ...
  test('[2] returns a copy, leaving the original object intact', () => {
    ...
    expect(utils.trimProperties(input)).toEqual({ name: 'jane' });
...
describe('[Exercise 2] trimPropertiesMutation', () => {
  test('[3] returns an object with the properties trimmed', () => {
    expect(utils.trimPropertiesMutation({ name: '  jane  ' })).toEqual({ name: 'jane' });
  })
  test('[4] the object returned is the exact same one we passed in', () => {
    const input = { name: '   jane   '};
    expect(utils.trimPropertiesMutation(input)).toEqual({ name: 'jane'});
    expect(input).toEqual({ name: 'jane' });
  })
})
...
describe('[Exercise 3] findLargestInteger', () => {
  test('[5] returns the largest number in an array of objects { integer: 2 }', () => {
    expect(utils.findLargestInteger([{ integer: 1 }, { integer: 3 }, { integer: 2 }])).toBe(3);
  })
})
...
describe('[Exercise 7] isEvenNumberAsync', () => {
  test('[19] resolves true if passed an even number', async () => {
    let even = await utils.isEvenNumberAsync(2);
    expect(even).toBe(true);
  })
  test('[20] resolves false if passed an odd number', async () => {
    let odd = await utils.isEvenNumberAsync(3);
